...
if __name__ == '__main__':
    ...
    trainset = Data.TensorDataset(X_train[1000:6000], y_train_genders[1000:6000])
    ...
    for epoch in range(Epoches):
        ...
        for step, (batch_x, batch_y) in enumerate(train_loader):
            ...
            loss = loss_func(output, truth2label(batch_y, 2))
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
    ...
    test_acc = np.sum((test_pred == np.array(y_test_genders[100:600]))) / float(y_test_genders[100:600].size(0))
